...
- ⚠ **打包风险（待验证）**：PyInstaller 默认可能不收集 rapidocr 的 .onnx 模型文件，打包版运行 OCR 可能报找不到模型。若 CI 打包后 OCR 失败，需在 spec 加 `collect_data_files('rapidocr_onnxruntime')` 或 `--collect-all rapidocr_onnxruntime`。本地源码运行（ai_api 环境）不受影响。
